...
class MiniExplosion {
  constructor(x, y, speedX, speedY) {
    ...
    if (colorChoice === "yellow-red") {
      this.color =
        Math.random() > 0.5 ? `hsl(50, 100%, 30%)` : `hsl(10, 100%, 30%)`;
    } else if (colorChoice === "blue-yellow") {
      this.color =
        Math.random() > 0.5 ? `hsl(200, 70%, 35%)` : `hsl(45, 100%, 30%)`;
    }
  }
...
function generateGreeting(name) {
  const greetings = [
    `Selamat Tahun Baru, ${name}! Semoga 2025 penuh dengan kebahagiaan, perkembangan, dan peluang tak terbatas untuk meraih impianmu.`,
    `Mari sambut awal yang baru, ${name}! Semoga tahun ini membawa kebahagiaan, cinta, dan keberanian untuk mengejar segala yang selalu kamu impikan.`,
    `Selamat Tahun 2025, ${name}! Semoga tahun ini membawa petualangan baru, kenangan tak terlupakan, dan kekuatan untuk menghadapi setiap tantangan.`,
    `Selamat datang di 2025, ${name}! Semoga tahun ini membuka pintu untuk pengalaman baru dan mendekatkanmu pada cita-citamu.`,
    `Inilah 2025, ${name}! Semoga tahun ini penuh dengan pencapaian baru, momen-momen bahagia, dan semangat untuk mengejar semua passionmu.`,
  ];
...
submitNameButton.addEventListener("click", () => {
  ...
  if (name) {
    ...
    greetingMessage.textContent =
      "Aku ada pesan sesuatu nih buat kamu Jadi silahkan di tulis dulu yah namanya";
  }
});
